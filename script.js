...
let orders = [];
let settings = {
    fabric: [{ name: 'Standard', price: 0 }, { name: 'Mesh', price: 50 }],
    jerseyType: [{ name: 'V-neck U-cut', price: 0 }, { name: 'Crew neck', price: 0 }],
    lowerType: [{ name: 'N/A', price: 0 }, { name: 'WITH LINING WITH POCKET', price: 50 }, { name: 'WITH POCKET', price: 50 }],
...
async function loadData() {
    ...
    } else { orders = []; console.error(ordersError); }
    ...
    } else { console.error(settingsError); }
    refreshAllDisplays();
}
...
async function saveOrders() {
    for (const order of orders) {
        ...
        const { error } = await supabaseClient.from('orders').upsert(dbOrder, { onConflict: 'id' });
        ...
        <div style="display:flex; align-items:center; gap:12px;">
            ...
            <div>
                ...
                <div style="color:#f97316;">
                    ...
                    ₱${order.totalPrice.toFixed(2)} | Down: ₱${order.downPayment}
                </div>
            </div>
        </div>
    </div>`;
...
async function confirmStatusChange(orderId, newStatus) {
    if (!isFullAccess()) { alert('View‑only mode: you cannot change order status.'); return; }
    let msg = '';
    ...
    if (confirm(msg)) await changeOrderStatus(orderId, newStatus);
}
async function changeOrderStatus(orderId, newStatus) {
    const order = orders.find(o => o.id === orderId);
    if(order) { order.status = newStatus; if (newStatus === 'completed') order.completedAt = new Date().toISOString(); await saveOrders(); }
}

async function deleteOrder(orderId) {
    if (!isFullAccess()) { alert('View‑only mode: you cannot delete orders.'); return; }
    const order = orders.find(o => o.id === orderId);
    if (!order) return;
    if (!confirm(`Delete order for "${order.customer}"? This action is permanent.`)) return;
    orders = orders.filter(o => o.id !== orderId);
    const { error } = await supabaseClient.from('orders').delete().eq('id', orderId);
    if (error) { console.error('Error deleting order:', error); alert('Failed to delete order.'); await loadData(); return; }
    await saveOrders();
    alert('✅ Order deleted.');
}
...
function renderOrdersByStatus() {
    ...
    if (tofileOrders.length === 0) { if (tofileGrid) tofileGrid.innerHTML = ''; if (tofileEmpty) tofileEmpty.classList.remove('hidden'); }
    else { if (tofileEmpty) tofileEmpty.classList.add('hidden'); if (tofileGrid) tofileGrid.innerHTML = tofileOrders.map(o => renderOrderCard(o, 'tofile')).join(''); }
    if (toprintOrders.length === 0) { if (toprintGrid) toprintGrid.innerHTML = ''; if (toprintEmpty) toprintEmpty.classList.remove('hidden'); }
    else { if (toprintEmpty) toprintEmpty.classList.add('hidden'); if (toprintGrid) toprintGrid.innerHTML = toprintOrders.map(o => renderOrderCard(o, 'toprint')).join(''); }
...
function openOrderModal(orderId) {
    ...
    setTimeout(() => {
        ...
        const wrapper = document.getElementById('garmentsTableWrapper');
        const icon = document.getElementById('garmentsToggleIcon');
        if (toggleBtn && wrapper) {
            let visible = true;
            toggleBtn.addEventListener('click', () => {
                if (visible) { wrapper.classList.add('hidden'); icon.classList.add('rotated'); }
                else { wrapper.classList.remove('hidden'); icon.classList.remove('rotated'); }
                visible = !visible;
            });
        }
    }, 50);
...
function renderEditForm(order, container, onSaveCallback) {
    ...
    function calculateEditTotals() {
        ...
        document.getElementById('editBalanceDueDisplay').innerText = '₱' + balanceDue.toFixed(2);
    ...
    document.querySelectorAll('#editGarmentsBody .remove-row').forEach(btn => { btn.onclick = () => { btn.closest('tr').remove(); calculateEditTotals(); }; });
    ...
    document.getElementById('editDownPayment').addEventListener('input', calculateEditTotals);
    ...
    editImageInput.addEventListener('change', (e) => {
        ...
        if(file && file.type.match('image.*')) {
            if(file.size > 2*1024*1024) { alert('Image too large'); editImageInput.value = ''; return; }
    ...
    document.getElementById('saveEditBtn').onclick = async () => {
        ...
        const fabricName = document.getElementById('editFabric').value;
        const fabricPrice = settings.fabric.find(f=>f.name===fabricName)?.price || 0;
        const jerseyTypePrice = settings.jerseyType.find(j=>j.name===document.getElementById('editJersey').value)?.price || 0;
        const lowerTypePrice = settings.lowerType.find(l=>l.name===document.getElementById('editLower').value)?.price || 0;
        let subTotalCalc = 0;
        updatedGarments.forEach(g => {
            if(g.garmentType === 'Jersey'){
                subTotalCalc += fabricPrice + jerseyTypePrice + lowerTypePrice;
        ...
        const discountAmount = parseFloat(document.getElementById('editDiscountAmount').value) || 0;
        const discountValue = Math.min(discountAmount, subTotalCalc);
        const totalPrice = subTotalCalc - discountValue;
        const downPayment = parseFloat(document.getElementById('editDownPayment').value) || 0;
        const balanceDue = Math.max(0, totalPrice - downPayment);
        const uniquePlayers = new Set();
        updatedGarments.forEach(g => { if (g.surname && g.surname.trim() !== '') uniquePlayers.add(`${g.surname.trim()}|${g.number?.trim()||''}`); });
        ...
        } else { await saveOrders(); onSaveCallback(); }
    };
...
function downloadCurrentReceipt() {
    ...
    html2canvas(currentReceiptElement, { scale: 2, backgroundColor: '#ffffff', logging: false, useCORS: true }).then(canvas => {
        ...
        link.click();
    }).catch(err => { console.error(err); alert('Could not generate receipt image.'); });
}
function getGarmentSummary(order) {
    ...
    order.garments.forEach(g => {
        ...
        if (g.garmentType === 'Jersey') {
            ...
            const gt = settings.garmentType.find(gt => gt.name === g.garmentType);
            unitPrice = (gt ? gt.price : 0) + (settings.fabric.find(f => f.name === order.fabric)?.price || 0);
        }
        const existing = summaryMap.get(g.garmentType);
        if (existing) { existing.quantity++; existing.totalPrice += unitPrice; }
        else { summaryMap.set(g.garmentType, { name: g.garmentType, quantity: 1, totalPrice: unitPrice }); }
    });
...
window.updateTotals = function() {
    ...
    rows.forEach(r => {
        ...
        if (surname && surname.trim() !== '') uniquePlayers.add(`${surname.trim()}|${number?.trim() || ''}`);
    });
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!isFullAccess()) { const settingsNav = document.querySelector('.nav-item[data-tab="settings"]'); if (settingsNav) settingsNav.style.display = 'none'; }
